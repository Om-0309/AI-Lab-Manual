...
# 1. LOAD DATA
# --------------------------------------
df = pd.read_csv(r"c:\Users\samsu\Downloads\AI Lab\AI Lab\cleaned.csv")
...
#    has_signature -> feature (X)
#    vtpercent -> target (y)
# --------------------------------------
df["has_signature"] = pd.to_numeric(df["has_signature"], errors="coerce")
df["vtpercent"] = pd.to_numeric(df["vtpercent"], errors="coerce")

print("\nNon-NaN counts BEFORE dropping:")
print("has_signature :", df["has_signature"].notna().sum())
print("vtpercent     :", df["vtpercent"].notna().sum())

# Use rows where BOTH are present
strict_df = df.dropna(subset=["has_signature", "vtpercent"])
print(f"\nRows with BOTH has_signature and vtpercent present: {len(strict_df)}")
...
print("\nLearned relationship (on scaled has_signature, but shown simply as):")
print(f"vtpercent ≈ {lr_intercept:.4f} + {lr_coef:.4f} * has_signature_scaled")
...
# --------------------------------------
joblib.dump(pipe_lr, "linear_regression_has_signature_vtpercent.pkl")
print("\nBaseline Linear Regression pipeline saved as linear_regression_has_signature_vtpercent.pkl")

# --------------------------------------
# 9. PREDICT FOR THE FULL CSV AND SAVE
# --------------------------------------
df_full = pd.read_csv(r"c:\Users\samsu\Downloads\AI Lab\AI Lab\cleaned.csv")
df_full["has_signature"] = pd.to_numeric(df_full["has_signature"], errors="coerce")

X_full = df_full[["has_signature"]]
...
output_file = r"c:\Users\samsu\Downloads\AI Lab\AI Lab\cleaned_with_vtpercent_pred.csv"
...
print(df_full[["has_signature", "vtpercent_pred_lr", "vtpercent_pred_ridge", "vtpercent_pred_lasso"]].head())
